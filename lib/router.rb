...
def ERB_FILE(file_name)
  return unless is_erb_file?(file_name)
...
class RouterResponse
  ...
  def response=(response)
    if @mime_type.nil? && response.length < 100 && string_is_file?(response) && File.exist?('dir/' + response)
...
class Router
  ...
  def match_route(route)
    ...
    if r_map
      if r_map[:METHOD] == :POST
        p 'THIS WAS A POST'
        rs = RouterResponse.new
        r_map[:BLOCK].call(route, rs)
        return rs
      end
